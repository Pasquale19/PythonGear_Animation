...
class EccentricCycloidGear:
    ...
    def __init__(self, z1: int, z2: int, a: float, **kwargs):
        ...
        self.c_star = kwargs.get('c_star', 0)
    ...
    def A(self,**kwargs):
        ...
        print(f"r_start={r_start:.1f} vs r1={self.r1:.1f}")
        a=self.a
        rA2=self.ra2
        alpha=acos((r_start**2+a**2-rA2**2)/(2*r_start*a))
        return r_start*np.array((cos(-alpha-pi/2),sin(-alpha-pi/2)))+np.array((0,self.a))
        angle=kwargs.get('rotation_angle',-pi/2-pi/(self.z1*2))
        M=self.e*np.array((sin(angle),cos(angle)))
        angle2=pi-self.phi_As
        return M+self.rA*np.array((sin(angle2),cos(angle2)))+np.array((0,self.a))
    ...
    def E(self,**kwargs):
        ...
        start point of mesh
        """
        rotation_angle=kwargs.get('rotation_angle',-pi/2-pi/(self.z1*2))
        a=self.a
        rA1=self.da1/2
        print(f"{rA1=} vs. {self.ra1}")
        rf2=self.df2/2
        beta=acos((a**2+rA1**2-rf2**2)/(2*a*rA1))
        angle=beta
        print(f"beta={beta*180/pi:.1f}")
        return np.array((0,a))+rA1*np.array((sin(angle),-cos(angle)))
    ...
    def calculate_arc_gear_geometry(self):
        ...
        # alpha=self.phi_rA/2
        # print(f"/n rA={self.rA}\t e={self.e} \t {self.rA/self.e=}")
        # print(f"alpha={alpha*180/pi:.1f}°")
        # beta=asin(sin(alpha)*self.e/self.rA)
        # beta=pi-beta
        # print(f"beta={beta*180/pi:.1f}°")
        #self.phi_As=pi-beta-alpha
        #self.phi_As=pi/3
        #q_F1 = e * sin(self.phi_s1 / 2) / sin((pi - self.phi_s1) / 2)
    ...
    def get_arc_profile2(self, rotation_angle: float = 0, num_points: int = 100, full_gear: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        ...
        def calculate_coords(angle: float, theta: float) -> Tuple[float, float]:
            a2 = -angle + theta
            return np.array((np.sin(theta), -np.cos(theta))) * self.e + np.array((0, self.a)) + np.array((-np.sin(a2), np.cos(a2))) * self.rA

        # Right flank
        coords = [calculate_coords(a, rotation_angle - phi_s1 / 2) for a in angles_right_flank]

        # Top point
        coords.append(np.array((np.sin(rotation_angle), -np.cos(rotation_angle))) * self.ra1 + (0, self.a))

        # Left flank
        coords.extend(calculate_coords(a, rotation_angle + phi_s1 / 2) for a in reversed(angles_right_flank))

        # Left foot
        phi_Fs = np.pi - np.pi / self.z1 - phi_As
        angles_foot = np.linspace(phi_Fs, -phi_Fs, num=max(1, num_points // 2), endpoint=False)
        theta = rotation_angle - np.pi / self.z1
        coords.extend((np.sin(theta), -np.cos(theta)) * self.q_F1 + (0, self.a) + 
                    (-np.sin(-a + theta), np.cos(-a + theta)) * self.rF1 for a in angles_foot)

        xy = np.array(coords).T - np.array([[0], [self.a]])

        if not full_gear:
            return xy[0] + self.a, xy[1]
    ...
    def get_cycloid_profile(self, num_points: int = 100) -> Tuple[Tuple[float, ...], Tuple[float, ...]]:
        """
        Generates the profile of the cycloid gear.
