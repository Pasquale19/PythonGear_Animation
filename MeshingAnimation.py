...
class GearMeshingPlot:
    ...
    def updateAngles(self,zeta:float):
        ...
        xy=calculate_arcPolygon(center=(0,self.gear.r2),radius=20,start_angle=0,end_angle=alpha)
    ...
    def key_press(self, event):
        if event.key == 'p':
            vis_F = self.ax_sliding_velocity.get_visible()
            self.ax_sliding_velocity.set_visible(not vis_F)
            self.fig.canvas.draw()
    ...
    def init_angle(self,zeta):
        ...
        xy=calculate_arcPolygon(center=(xt,yt),radius=20,start_angle=0,end_angle=alpha)
        #self.arc_alpha,=ax.plot(x,y,color="black")
        self.arc_alpha=Polygon(xy, closed=True, color='black',fill="grey",facecolor="grey",edgecolor="black",alpha=0.5)
