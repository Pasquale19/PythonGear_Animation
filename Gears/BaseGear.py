...
import numpy as np
#from Gears.InvoluteGear2D import Gear2D,InvoluteGear2D,InvoluteGear2D2
from InvoluteGear2D import Gear2D,InvoluteGear2D,InvoluteGear2D2
...
class InvoluteGear():
    ...
    def calculate_parameter(self):
        ...
        self.r1=abs(z1*m/2)
        self.r2=abs(z2*m/2)
        self.rw1=abs(m*z1/2*cos(alpha)/cos(a_w))
        self.rw2=abs(m*z2/2*cos(alpha)/cos(a_w))
        self.rb1=abs(m*z1/2*cos(alpha))
        self.rb2=abs(m*z2/2*cos(alpha))
        self.ra1=abs(m*z1/2+m*(hap1+x1))
        self.rf1=abs(m*(z1/2-hfp1+x1))
        ...
        alpha_kopf=acos(abs(self.r1/self.ra1)*cos(self.alpha)) #eingriffswinkel am Kopf
        alpha_fuss=acos(abs(self.r1/self.r1)*cos(self.alpha)) #eingriffswinkel am Kopf
    ...
    def gearGeometry1(self,num:int=1000,**kwargs):
        ...
        x,y=InvoluteGear2D(z=self.z1,m=self.m,alpha=self.alpha,x=self.x1,rotation_angle=gamma,M=np.array((0,0)),hap=self.hap1,hfp=self.hfp1)
        return x,y
    
    def gearGeometry2(self,num:int=1000,**kwargs):
        gamma=kwargs.get("rotation_angle",pi/(2*self.z2)+pi/2)
        x,y=InvoluteGear2D2(z=self.z2,m=self.m,alpha=self.alpha,x=self.x2,rotation_angle=gamma,M=self.center2,hap=self.hap2,hfp=self.hfp2)
        return x,y
